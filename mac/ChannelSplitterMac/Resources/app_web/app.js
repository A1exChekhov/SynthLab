...
function meterLoop(){
  ...
  API.meters().then(m=>{
    ...
    if(_radioUrl && _radioStation && m.radio_active){   // радио — станция + трек из ICY, а не системный now-playing
      ...
      // держим последние данные выбранного источника (никаких чужих обложек/названий).
      const want=src0.lb_name||'';
      if(want && m.np_app && m.np_app!==want){ /* чужой плеер — игнор */ }
      else updateNP(m.np||{ title:'', sub:'', cur:'', total:'', posfrac:0, art_id:'' });
